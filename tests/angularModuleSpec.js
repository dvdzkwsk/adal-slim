...
describe("TaskCtl", function () {
    ...
    beforeEach(
        angular.mock.inject(function (
            ...
            controller("TaskCtl", {
                $scope: scope,
                adalAuthenticationService: adalServiceProvider,
            })
    ...
    it("send tokens for webapi call in endpoints list", function () {
        $httpBackend
            .expectGET("/api/Todo/5", function (headers) {
    ...
    it("send tokens for webapi call in endpoints list", function () {
        $httpBackend
            .expectGET("/anotherApi/Item/13", function (headers) {
    ...
    it("send tokens for webapi call in endpoints list", function () {
        $httpBackend
            .expectGET("https://testapi.com/", function (headers) {
                return headers.Authorization === "Bearer Token3434"
            })
            .respond(200)
        scope.taskCall3()
    ...
    it("does not send tokens for webapi(https) call not in endpoints list", function () {
        $httpBackend
            .expectGET("https://test.com/", function (headers) {
    ...
    it("does not send tokens for webapi(http) call not in endpoint list", function () {
        $httpBackend
            .expectGET("http://testwebapi.com/", function (headers) {
                return headers.hasOwnProperty("Authorization") === false
            })
            .respond(200)
        scope.taskCall6()
    ...
    it("send tokens for app backend call not in endpoints list", function () {
        $httpBackend
            .expectGET("/someapi/item", function (headers) {
    ...
    it("send tokens for app backend call", function () {
        $httpBackend
            .expectGET("https://myapp.com/someapi/item", function (headers) {
                return headers.Authorization === "Bearer Token456"
            })
            .respond(200)
        scope.taskCall5()
    ...
    it("tests errorResponse broadcast when login is in progress", function () {
        ...
        expect(msg.data).toBe(
            "login in progress, cancelling the request for https://myapp.com/someapi/item",
        )
    })
    ...
    it("tests login failure after users logs in", function () {
        var mockInvalidClientIdToken =
            "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhdWQiOiJjbGllbnQxMjMiLCJuYW1lIjoiSm9obiBEb2UiLCJ1cG4iOiJqb2huQGVtYWlsLmNvbSJ9.zNX4vfLzlbFeKHZ9BMN3sYLtEEE-0o1RoL4NUhXz-l8"
        location.hash(
            "#" + "id_token=" + mockInvalidClientIdToken + "&state=1234",
        )
        ...
        expect(errorDesc).toBe(
            "Invalid id_token. id_token: " + mockInvalidClientIdToken,
        )
    ...
    it("tests login success after users logs in", function () {
        var mockIdToken =
            "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhdWQiOiJjbGllbnRpZDEyMyIsIm5hbWUiOiJKb2huIERvZSIsInVwbiI6ImpvaG5AZW1haWwuY29tIiwibm9uY2UiOiIxMjM0In0.bpIBG3n1w7Cv3i_JHRGji6Zuc9F5H8jbDV5q3oj0gcw"
    ...
    it("tests route change handler", function () {
        ...
        expect(adalServiceProvider.config.anonymousEndpoints).toContain(
            aboutRoute.templateUrl,
        )
        $httpBackend.flush()
    })
...
describe("StateCtrl", function () {
    ...
    it("checks if anonymous endpoints are populated on statechange event if states are nested and separated by .", function () {
        ...
        for (var i = 0; i < states.length; i++) {
            expect(adalServiceProvider.config.anonymousEndpoints[i]).toEqual(
                states[i] + ".html",
            )
        }
    })
...
describe("AcquireTokenCtl", function () {
    ...
    it("checks if acquireTokenSuccess/acquireTokenFailure events are broadcasted in case of acquireToken", function () {
        ...
        adalServiceProvider.acquireToken(
            adalServiceProvider.config.loginResource,
        )
    ...
    it("tests auto id token renew when id token expires", function () {
        ...
        store = {
            "adal.idtoken":
                "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhdWQiOiJjbGllbnRpZDEyMyIsIm5hbWUiOiJKb2huIERvZSIsInVwbiI6ImpvaG5AZW1haWwuY29tIiwibm9uY2UiOm51bGx9.DLCO6yIWhnNBYfHH8qFPswcH4M2Alpjn6AZy7K6HENY",
        }
        scope.$apply()
