...
describe("Adal", function () {
    ...
    var IDTOKEN_MOCK =
        "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsIng1dCI6IjVUa0d0S1JrZ2FpZXpFWTJFc0xDMmdPTGpBNCJ9..WHsl8TH1rQ3dQbRkV0TS6GBVAxzNOpG3nGG6mpEBCwAOCbyW6qRsSoo4qq8I5IGyerDf2cvcS-zzatHEROpRC9dcpwkRm6ta5dFZuouFyZ_QiYVKSMwfzEC_FI-6p7eT8gY6FbV51bp-Ah_WKJqEmaXv-lqjIpgsMGeWDgZRlB9cPODXosBq-PEk0q27Be-_A-KefQacJuWTX2eEhECLyuAu-ETVJb7s19jQrs_LJXz_ISib4DdTKPa7XTBDJlVGdCI18ctB67XwGmGi8MevkeKqFI8dkykTxeJ0MXMmEQbE6Fw-gxmP7uJYbZ61Jqwsw24zMDMeXatk2VWMBPCuhA"
    var STATE = "33333333-3333-4333-b333-333333333333"
    var SESSION_STATE = "451c6916-27cf-4eae-81cd-accf96126398"
    var VALID_URLFRAGMENT =
        "id_token=" +
        IDTOKEN_MOCK +
        "" +
        "&state=" +
    ...
    beforeEach(function () {
        ...
        storageFake.clear()
        storageFake.setItem(
            STORAGE_ACCESS_TOKEN_KEY + RESOURCE1,
            "access_token_in_cache" + RESOURCE1,
        )
        var secondsNow = mathMock.round(0)
        storageFake.setItem(
            STORAGE_EXPIRATION_KEY + RESOURCE1,
            secondsNow + SECONDS_TO_EXPIRE,
        ) // seconds to expire
    ...
    it("says token expired", function () {
        ...
        expect(adal.getCachedToken(RESOURCE1)).toEqual(
            "access_token_in_cache" + RESOURCE1,
        )
    ...
    it("navigates user to login by default", function () {
        ...
        adal.login()
        expect(adal.promptUser).toHaveBeenCalledWith(
            DEFAULT_INSTANCE +
                conf.tenant +
                "/oauth2/authorize?response_type=id_token&client_id=client&redirect_uri=contoso_site&state=33333333-3333-4333-b333-333333333333" +
    ...
    it("calls displaycall if given for login", function () {
        ...
        adal.login()
        expect(adal.config.displayCall).toHaveBeenCalledWith(
            DEFAULT_INSTANCE +
                conf.tenant +
                "/oauth2/authorize?response_type=id_token&client_id=client&redirect_uri=contoso_site&state=33333333-3333-4333-b333-333333333333" +
                "&client-request-id=33333333-3333-4333-b333-333333333333" +
    ...
    it("attempts to renew if token expired and renew is allowed", function () {
        ...
        waitsFor(
            function () {
                return (
                    mockFrames["adalRenewFrame" + RESOURCE1].src !==
                    "about:blank"
                )
            },
            "iframe src not updated",
            2000,
        )

        runs(function () {
            expect(mockFrames["adalRenewFrame" + RESOURCE1].src).toBe(
                DEFAULT_INSTANCE +
                    conf.tenant +
                    "/oauth2/authorize?response_type=token&client_id=client&resource=" +
                    RESOURCE1 +
                    "&redirect_uri=contoso_site&state=33333333-3333-4333-b333-333333333333%7Ctoken.resource1" +
                    "&client-request-id=33333333-3333-4333-b333-333333333333" +
                    adal._addLibMetadata() +
                    "&prompt=none&login_hint=test%40testuser.com&domain_hint=testuser.com",
            )
        })
    })
    ...
    it("allows multiple callers to be notified when the token is renewed. Also checks if all registered acquireToken callbacks are called in the case when one of the callbacks throws an error", function () {
        ...
        adal._callBackMappedToRenewStates[adal.config.state](
            null,
    ...
    it("clears cache", function () {
        ...
        storageFake.setItem(
            adal.CONSTANTS.STORAGE.TOKEN_KEYS,
            "key1|key2|" + RESOURCE1 + "|",
        )
        storageFake.setItem(
            adal.CONSTANTS.STORAGE.ACCESS_TOKEN_KEY + "key1",
            "value1",
        )
        storageFake.setItem(
            adal.CONSTANTS.STORAGE.ACCESS_TOKEN_KEY + "key2",
            "value2",
        )
        storageFake.setItem(
            adal.CONSTANTS.STORAGE.SESSION_STATE,
            "session_state",
        )
        storageFake.setItem(adal.CONSTANTS.STORAGE.STATE_LOGIN, "state login")
        storageFake.setItem(adal.CONSTANTS.STORAGE.ERROR, "error")
        storageFake.setItem(
            adal.CONSTANTS.STORAGE.ERROR_DESCRIPTION,
            "error description",
        )
        adal.clearCache()
        var store = storageFake.storeVerify()
        for (var prop in store) {
            expect(store[prop] === "" || store[prop] == 0 || !store[prop]).toBe(
                true,
            )
        }
    })
    ...
    it("clears cache for a resource", function () {
        ...
        for (var prop in store) {
            if (
                prop == adal.CONSTANTS.STORAGE.ACCESS_TOKEN_KEY + RESOURCE1 ||
                prop == adal.CONSTANTS.STORAGE.EXPIRATION_KEY + RESOURCE1
            ) {
                expect(
                    store[prop] === "" || store[prop] == 0 || !store[prop],
                ).toBe(true)
            }
        }
        var item = adal.CONSTANTS.STORAGE.ACCESS_TOKEN_KEY + "key1"
        expect(store[item] === "" || store[item] == 0 || !store[item]).toBe(
            false,
        )
    })
    ...
    it("has logout redirect if given", function () {
        ...
        expect(adal.promptUser).toHaveBeenCalledWith(
            DEFAULT_INSTANCE +
                adal.config.tenant +
                "/oauth2/logout?post_logout_redirect_uri=https%3A%2F%2Fcontoso.com%2Flogout",
        )
    })
    ...
    it("uses common for tenant if not given at logout redirect", function () {
        ...
        expect(adal.promptUser).toHaveBeenCalledWith(
            DEFAULT_INSTANCE +
                "common/oauth2/logout?post_logout_redirect_uri=https%3A%2F%2Fcontoso.com%2Flogout",
        )
    })
    ...
    it("uses logout uri if given", function () {
        ...
        adal.config.displayCall = null
        adal.config.clientId = "client"
        adal.config.logOutUri =
            "https://login.microsoftonline.com/adfs/ls/?wa=wsignout1.0"
        spyOn(adal, "promptUser")
        adal.logOut()
        expect(adal.promptUser).toHaveBeenCalledWith(
            "https://login.microsoftonline.com/adfs/ls/?wa=wsignout1.0",
    ...
    it("gets request info from hash", function () {
        ...
        requestInfo = adal.getRequestInfo(
            "#error_description=someting_wrong&state=1232",
        )
        expect(requestInfo.valid).toBe(true)
        ...
        checkStateType(
            adal.CONSTANTS.STORAGE.STATE_LOGIN,
            "1234",
            adal.REQUEST_TYPE.LOGIN,
        )
    })
    ...
    var checkStateType = function (state, stateExpected, requestType) {
        ...
        var requestInfo = adal.getRequestInfo(
            "#error_description=someting_wrong&state=" + stateExpected,
        )
        expect(requestInfo.valid).toBe(true)
    ...
    it("saves errors token from callback", function () {
        ...
        expect(storageFake.getItem(adal.CONSTANTS.STORAGE.ERROR)).toBe(
            "invalid",
        )
        expect(
            storageFake.getItem(adal.CONSTANTS.STORAGE.ERROR_DESCRIPTION),
    ...
    it("saves token if state matches", function () {
        ...
        expect(
            storageFake.getItem(
                adal.CONSTANTS.STORAGE.ACCESS_TOKEN_KEY + "loginResource1",
            ),
        ).toBe("token123")
    })
    ...
    it("saves expiry if state matches", function () {
        ...
        expect(
            storageFake.getItem(
                adal.CONSTANTS.STORAGE.EXPIRATION_KEY + "loginResource1",
            ),
        ).toBe(mathMock.round(1) + 3589)
    })
    ...
    it("does not save user for invalid nonce in idtoken", function () {
        var requestInfo = {
            ...
            parameters: {
                id_token:
                    "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsIng1dCI6IjVUa0d0S1JrZ2FpZXpFWTJFc0xDMmdPTGpBNCJ9..GzbTwMXhjs4uJFogd1B46C_gKX6uZ4BfgJIpzFS-n-HRXEWeKdZWboRC_-C4UnEy6G9kR6vNFq7zi3DY1P8uf1lUavdOFUE27xNY1McN1Vjm6HKxKNYOLU549-wIb6SSfGVycdyskdJfplf5VRasMGclwHlY0l9bBCTaPunjhfcg-mQmGKND-aO0B54EGhdGs740NiLMCh6kNXbp1WAv7V6Yn408qZEIsOQoPO0dW-wO54DTqpbLtqiwae0pk0hDxXWczaUPxR_wcz0f3TgF42iTp-j5bXTf2GOP1VPZtN9PtdjcjDIfZ6ihAVZCEDB_Y9czHv7et0IvB1bzRWP6bQ",
                state: "123",
            },
            stateMatch: true,
            stateResponse: "123",
    ...
    it("saves null for username if idtoken is invalid", function () {
        ...
        expect(
            storageFake.getItem(adal.CONSTANTS.STORAGE.USERNAME),
        ).toBeUndefined()
    })
    ...
    it("test get resource for endpoint from app backend", function () {
        ...
        expect(adal.getResourceForEndpoint("https://host.com")).toBe(
            adal.config.loginResource,
        )
        expect(adal.getResourceForEndpoint("https://host.com/a/b")).toBe(
            adal.config.loginResource,
        )
        expect(adal.getResourceForEndpoint("https://host.com/page/")).toBe(
            adal.config.loginResource,
        )
        expect(adal.getResourceForEndpoint("https://notapp.com/page/")).toBe(
            null,
        )
        expect(adal.getResourceForEndpoint("/api/todo")).toBe(
            adal.config.loginResource,
        )
    })
    ...
    it("test host extraction", function () {
        ...
        expect(adal._getHostFromUri("http://a.com/")).toBe("a.com")
        expect(adal._getHostFromUri("http://localhost:8080")).toBe(
            "localhost:8080",
        )
    })
    ...
    it("saves error if state mismatch", function () {
        ...
        expect(
            storageFake.getItem(adal.CONSTANTS.STORAGE.ERROR_DESCRIPTION),
        ).toBe("Invalid_state. state: " + requestInfo.stateResponse)
    })
    ...
    it("tests the load frame timeout method", function () {
        ...
        expect(
            storageFake.getItem(
                adal.CONSTANTS.STORAGE.RENEW_STATUS + RESOURCE1,
            ),
        ).toBe(adal.CONSTANTS.TOKEN_RENEW_STATUS_IN_PROGRESS)
        ...
        waitsFor(
            function () {
                return (
                    storageFake.getItem(
                        adal.CONSTANTS.STORAGE.RENEW_STATUS + RESOURCE1,
                    ) === adal.CONSTANTS.TOKEN_RENEW_STATUS_CANCELED
                )
            },
            "token renew status not updated",
            1000,
        )
        ...
        runs(function () {
            ...
            ).toBe(adal.CONSTANTS.TOKEN_RENEW_STATUS_CANCELED)
            ...
            ).toBe(adal.CONSTANTS.TOKEN_RENEW_STATUS_COMPLETED)
        })
    })
    ...
    it("attempts to renewidToken if token expired and renew is allowed", function () {
        ...
        expect(storageFake.getItem(adal.CONSTANTS.STORAGE.NONCE_IDTOKEN)).toBe(
            "33333333-3333-4333-b333-333333333333",
        )
        expect(adal.config.state).toBe(
            "33333333-3333-4333-b333-333333333333" + "|" + "client",
        )
        expect(adal._renewStates.length).toBe(1)
        // Wait for initial timeout load
        console.log("Waiting for initial timeout")
        ...
        runs(function () {
            expect(mockFrames["adalIdTokenFrame"].src).toBe(
                DEFAULT_INSTANCE +
                    conf.tenant +
                    "/oauth2/authorize?response_type=id_token&client_id=" +
                    adal.config.clientId +
                    "&redirect_uri=contoso_site&state=33333333-3333-4333-b333-333333333333%7Cclient" +
                    "&client-request-id=33333333-3333-4333-b333-333333333333" +
                    adal._addLibMetadata() +
                    "&prompt=none&login_hint=test%40testuser.com&domain_hint=testuser.com" +
                    "&nonce=33333333-3333-4333-b333-333333333333",
    ...
    it("tests handleWindowCallback function for RENEW_TOKEN", function () {
        ...
        adal.getRequestInfo = function (hash) {
            return {
                ...
                parameters: {
                    error_description: "error description",
                    error: "invalid",
                    id_token: IDTOKEN_MOCK,
                    session_state: "61ae5247-eaf8-4496-a667-32b0acbad7a0",
                    state: "19537a2a-e9e7-489d-ae7d-3eefab9e4137",
                },
                stateMatch: true,
        ...
        adal._callBackMappedToRenewStates[
            adal.getRequestInfo().stateResponse
        ] = callback
    ...
    it("tests handleWindowCallback function for LOGIN_REQUEST", function () {
        ...
        storageFake.setItem(
            adal.CONSTANTS.STORAGE.LOGIN_REQUEST,
            "www.test.com",
        )
        window.parent = {}
        window.parent._adalInstance = adal
    ...
    it("use the same correlationId for each request sent to AAD if set by user", function () {
        ...
        expect(adal._loadFrameTimeout).toHaveBeenCalledWith(
            DEFAULT_INSTANCE +
                ...
                adal._addLibMetadata() +
                "&prompt=none&login_hint=test%40testuser.com&domain_hint=testuser.com",
            "adalRenewFrametoken.resource1",
            "token.resource1",
        )

        adal._activeRenewals = {}
        adal._user = {
            profile: {sub: "test@testuser.com"},
            userName: "test@domain.com",
        }
        adal.acquireToken(RESOURCE1, callback)
        expect(adal._loadFrameTimeout).toHaveBeenCalledWith(
    ...
    it("generates new correlationId for each request sent to AAD if not set by user", function () {
        ...
        expect(adal._loadFrameTimeout).toHaveBeenCalledWith(
            DEFAULT_INSTANCE +
                ...
                "/oauth2/authorize?response_type=token&client_id=client&resource=" +
                RESOURCE1 +
                "&redirect_uri=contoso_site&state=11111111-1111-4111-9111-111111111111%7Ctoken.resource1" +
        ...
        expect(adal._loadFrameTimeout).toHaveBeenCalledWith(
            DEFAULT_INSTANCE +
                ...
                "/oauth2/authorize?response_type=token&client_id=client&resource=" +
                RESOURCE1 +
                "&redirect_uri=contoso_site&state=44444444-4444-4444-8444-444444444444%7Ctoken.resource1" +
                "&client-request-id=44444444-4444-4444-8444-444444444444" +
                adal._addLibMetadata() +
                "&prompt=none",
            "adalRenewFrametoken.resource1",
            "token.resource1",
    ...
    it("tests if error parameter is passed to acquireToken callback", function () {
        var errorHash =
            "#error=interaction_required&error_description=some_description&state=someState"
        var errDesc = "",
            token = "",
            err = ""
        var callback = function (valErrDesc, valToken, valErr) {
    ...
    it("tests if error is logged and code flow is completed when there is a failure in the user defined callback function in case of login", function () {
        ...
        adal.handleWindowCallback()
        expect(window.logMessage).toContain(
    ...
    it("removes the prompt query parameter user provided", function () {
        var url =
            "https://login.onmicrosoft.com?prompt=none&client_id=12345&response_type=id_token"
        var newUrl = adal._urlRemoveQueryStringParameter(url, "prompt")
        expect(newUrl).toBe(
            "https://login.onmicrosoft.com?client_id=12345&response_type=id_token",
        )

        url =
            "https://login.onmicrosoft.com?client_id=12345&prompt=none&response_type=id_token"
        ...
        url =
            "https://login.onmicrosoft.com?client_id=12345&response_type=id_token&prompt=none"
        newUrl = adal._urlRemoveQueryStringParameter(url, "prompt")
        expect(newUrl).toBe(
            "https://login.onmicrosoft.com?client_id=12345&response_type=id_token",
        )
    ...
    it("to add sid=<sid value> instead of login_hint=<upn value> if sid is present in the id_token response received from the server ", function () {
        ...
        expect(newUrl).toBe(
            "https://login.onmicrosoft.com&prompt=none" +
                "&sid=" +
                encodeURIComponent(adal._user.profile.sid),
        )
        ...
        expect(newUrl).toBe(
            "https://login.onmicrosoft.com" +
                "&login_hint=" +
                encodeURIComponent(adal._user.profile.upn) +
                "&domain_hint=" +
                encodeURIComponent(adal._user.profile.upn.split("@")[1]),
        )
        adal._user = null
    })
    ...
    it("checks Logger to see if pii messages are logged when piiLogging is disabled by the developer", function () {
        ...
        adal.promptUser(
            "https://login.microsoftonline.com/common/oauth2/v2.0/authorize?response_type=token&state=9ff87e68-76a6-4537-9b2a-9313da6c576b&nonce=d503ae2c-51fc-447b-8b44-a0aed28033b8",
        )
